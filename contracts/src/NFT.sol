...
contract NFT is ERC1155Supply, AccessControl {
    ...
    function getRoles(address account) public view returns (bool isRecordCompany, bool isWe, bool isAdmin) {
        isRecordCompany = hasRole(RECORD_COMPANY_ROLE, account);
        isWe = hasRole(WE_ROLE, account);
        isAdmin = hasRole(DEFAULT_ADMIN_ROLE, account);
        return (isRecordCompany, isWe, isAdmin);
